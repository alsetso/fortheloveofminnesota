...
type ChatAccountUsageModalProps = {
  open: boolean;
  onClose: () => void;
  loading: boolean;
  error: string | null;
  data: AccountUsageResponse | null;
};
...
/**
 * Account usage sheet — totals across all chats for the signed-in account.
 */
export default function ChatAccountUsageModal({
  open,
  onClose,
  loading,
  error,
  data,
}: ChatAccountUsageModalProps) {
  const titleId = useId();
  ...
  const empty =
    ...
    (account.thread_count === 0 &&
      ...
      account.turn_count === 0 &&
      account.total_tokens === 0);

  const activityHint =
    account?.first_at && account?.last_at
      ? `${new Date(account.first_at).toLocaleDateString(undefined, {
          month: 'short',
          day: 'numeric',
          year: 'numeric',
        })} – ${new Date(account.last_at).toLocaleDateString(undefined, {
          month: 'short',
          day: 'numeric',
          year: 'numeric',
        })}`
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
          {loading ? (
            ...
            <p className="py-8 text-center text-[14px] text-foreground-muted">
              No chat usage on this account yet.
            </p>
          ) : (
            <>
              <div className="grid grid-cols-2 gap-x-4 gap-y-4">
                <Stat
                  label="Threads"
                  value={formatCount(account.thread_count)}
                />
                <Stat
                  label="Messages"
                  value={formatCount(account.message_count)}
                />
                <Stat
                  label="Total tokens"
                  value={formatTokenCount(account.total_tokens)}
                  hint={`${account.turn_count} model turn${account.turn_count === 1 ? '' : 's'}`}
                />
                <Stat
                  label="Time"
                  value={formatDurationMs(account.duration_ms)}
                  hint={activityHint ?? 'Model + tools'}
                />
                <Stat
                  label="Input"
                  value={formatTokenCount(account.input_tokens)}
                />
                <Stat
                  label="Output"
                  value={formatTokenCount(account.output_tokens)}
                />
                <Stat
                  label="Reasoning"
                  value={formatTokenCount(account.reasoning_tokens)}
                />
                <Stat
                  label="Cached"
                  value={formatTokenCount(account.cached_tokens)}
                  hint={
                    account.cache_write_tokens > 0
                      ? `${formatTokenCount(account.cache_write_tokens)} written`
                      : undefined
                  }
                />
                <Stat
                  label="Web search"
                  value={String(account.web_search_call_count)}
                  hint={`${account.web_search_turns} turn${account.web_search_turns === 1 ? '' : 's'}`}
                />
              </div>
