...
COMMENT ON COLUMN public.map_shares.permission IS 'Permission level: view (read-only) or edit (can add/modify/delete points)';

COMMENT ON TYPE public.map_permission IS 'Permission level for map sharing: view (read-only) or edit (can modify points)';

COMMENT ON FUNCTION public.user_owns_map(UUID) IS 'Checks if the current authenticated user owns the specified map. Uses SECURITY DEFINER to bypass RLS on accounts table.';
