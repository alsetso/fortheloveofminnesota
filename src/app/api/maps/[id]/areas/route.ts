 ...
 * List areas on a map (supports both UUID and custom_slug)
 * 
 * Security:
  ...
  return withSecurity(
    ...
    async (req, { userId, accountId }) => {
      try {
        ...
        // Fetch areas (RLS will filter based on permissions)
        ...
        if (error) {
          if (process.env.NODE_ENV === 'development') {
            console.error('[Maps API] Error fetching areas:', error);
          }
  ...
  return withSecurity(
    ...
    async (req, { userId, accountId }) => {
      try {
        ...
        if (areaError) {
          if (process.env.NODE_ENV === 'development') {
            console.error('[Maps API] Error creating area:', areaError);
          }
