-- Recreate map_areas table for custom maps
-- Areas are polygon/multipolygon shapes drawn on maps
...
CREATE INDEX IF NOT EXISTS idx_map_areas_map_id ON public.map_areas(map_id);
CREATE INDEX IF NOT EXISTS idx_map_areas_created_at ON public.map_areas(created_at DESC);

-- Create GIN index for geometry queries
CREATE INDEX IF NOT EXISTS idx_map_areas_geometry ON public.map_areas USING GIN (geometry);
  ...
  WITH CHECK (
    ...
    AND EXISTS (
      ...
      AND (
        ...
        OR
        -- Public maps allow anyone to create areas
        map.visibility = 'public'
      )
    )
  );
...
COMMENT ON TABLE public.map_areas IS 'Polygon areas on custom maps with GeoJSON coordinates';
