-- Unify mentions into map_pins table
-- Migrates all mentions to map_pins, linking them to the "live" map
-- Preserves all functionality: likes, collections, types, media, etc.

-- ============================================================================
-- STEP 1: Find or create the "live" map
-- ============================================================================

-- Find or create live map
DO $$
DECLARE
  live_map_id UUID;
...
BEGIN
  -- Try to find existing live map by slug
  SELECT id INTO live_map_id
  FROM public.map
  WHERE slug = 'live'
  LIMIT 1;

  -- If not found, try custom_slug (legacy)
  ...
  IF live_map_id IS NULL THEN
    ...
    -- If no admin, use first account (fallback)
    ...
    -- Check if 'name' column exists (new structure)
    SELECT EXISTS (
    ...
    -- Create live map (handle both old and new column names)
    IF has_name_column THEN
      -- New structure (name, slug)
    ...
    ELSE
      -- Old structure (title, custom_slug)
      INSERT INTO public.map (
        account_id,
        title,
        custom_slug,
    ...
    RAISE NOTICE 'Created live map with ID: %', live_map_id;
  ELSE
    RAISE NOTICE 'Found existing live map with ID: %', live_map_id;
  END IF;
END $$;

-- ============================================================================
-- STEP 2: Expand map_pins table to include all mentions columns
-- ============================================================================

-- Add account_id (for ownership/RLS)
ALTER TABLE public.map_pins
  ADD COLUMN IF NOT EXISTS account_id UUID REFERENCES public.accounts(id) ON DELETE CASCADE;

-- Add visibility (public/only_me)
DO $$ BEGIN
  CREATE TYPE public.map_pin_visibility AS ENUM ('public', 'only_me');
EXCEPTION
  WHEN duplicate_object THEN null;
END $$;

ALTER TABLE public.map_pins
  ADD COLUMN IF NOT EXISTS visibility public.map_pin_visibility NOT NULL DEFAULT 'public';

-- Add archived (soft delete)
ALTER TABLE public.map_pins
  ADD COLUMN IF NOT EXISTS archived BOOLEAN NOT NULL DEFAULT false;

-- Add post_date (for year filtering)
ALTER TABLE public.map_pins
  ADD COLUMN IF NOT EXISTS post_date TIMESTAMP WITH TIME ZONE;

-- Add description (mentions have description, map_pins have caption - keep both)
-- map_pins.caption stays for custom map pins, description for migrated mentions
ALTER TABLE public.map_pins
...
-- Add media (map_pins already has image_url, video_url - add media_type)
DO $$ BEGIN
  CREATE TYPE public.map_pin_media_type AS ENUM ('image', 'video', 'none');
EXCEPTION
  WHEN duplicate_object THEN null;
END $$;

ALTER TABLE public.map_pins
  ADD COLUMN IF NOT EXISTS media_type public.map_pin_media_type NOT NULL DEFAULT 'none';
...
-- Add is_active (for soft delete, similar to archived)
ALTER TABLE public.map_pins
...
CREATE INDEX IF NOT EXISTS idx_map_pins_account_id ON public.map_pins(account_id) WHERE account_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_map_pins_visibility ON public.map_pins(visibility) WHERE visibility IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_map_pins_archived ON public.map_pins(archived) WHERE archived = false;
CREATE INDEX IF NOT EXISTS idx_map_pins_post_date ON public.map_pins(post_date) WHERE post_date IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_map_pins_city_id ON public.map_pins(city_id) WHERE city_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_map_pins_map_meta ON public.map_pins USING GIN (map_meta) WHERE map_meta IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_map_pins_atlas_meta ON public.map_pins USING GIN (atlas_meta) WHERE atlas_meta IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_map_pins_collection_id ON public.map_pins(collection_id) WHERE collection_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_map_pins_mention_type_id ON public.map_pins(mention_type_id) WHERE mention_type_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_map_pins_view_count ON public.map_pins(view_count DESC) WHERE view_count > 0;
CREATE INDEX IF NOT EXISTS idx_map_pins_tagged_account_ids ON public.map_pins USING GIN (tagged_account_ids) WHERE tagged_account_ids IS NOT NULL AND jsonb_array_length(tagged_account_ids) > 0;
CREATE INDEX IF NOT EXISTS idx_map_pins_is_active ON public.map_pins(is_active) WHERE is_active = true;

-- Composite indexes for common queries
CREATE INDEX IF NOT EXISTS idx_map_pins_map_id_visibility ON public.map_pins(map_id, visibility) WHERE archived = false AND is_active = true;
CREATE INDEX IF NOT EXISTS idx_map_pins_map_id_account_id ON public.map_pins(map_id, account_id) WHERE account_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_map_pins_account_id_visibility ON public.map_pins(account_id, visibility) WHERE archived = false;

-- ============================================================================
-- STEP 4: Add constraints
-- ============================================================================

-- Ensure tagged_account_ids is always an array
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'map_pins_tagged_account_ids_is_array'
    AND conrelid = 'public.map_pins'::regclass
  ) THEN
    ALTER TABLE public.map_pins
      ADD CONSTRAINT map_pins_tagged_account_ids_is_array 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'map_pins_view_count_non_negative'
    AND conrelid = 'public.map_pins'::regclass
  ) THEN
    ALTER TABLE public.map_pins
      ADD CONSTRAINT map_pins_view_count_non_negative 
      CHECK (view_count >= 0);
  END IF;
END $$;
...
SELECT 
  m.id,
  COALESCE(
    (SELECT id FROM public.map WHERE slug = 'live' LIMIT 1),
    (SELECT id FROM public.map WHERE custom_slug = 'live' LIMIT 1)
  ) as map_id,
  m.lat,
  m.lng,
  m.account_id,
  ...
  NOT m.archived as is_active, -- Active if not archived
  m.created_at,
  m.updated_at
...
-- ============================================================================
-- STEP 6: Update mentions_likes to reference map_pins
-- ============================================================================
...
-- ============================================================================
-- STEP 10: Force PostgREST schema cache refresh
-- ============================================================================
