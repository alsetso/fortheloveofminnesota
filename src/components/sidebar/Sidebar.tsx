    ...
    <>
      ...
      <aside className="hidden lg:flex relative w-16 flex-shrink-0 flex-col h-full bg-white border-r border-gray-200">
        ...
        <div className="p-2 border-t border-gray-200 space-y-2">
          {account && (
            <>
              {/* Upgrade button - only show for hobby plan */}
              {account.plan === 'hobby' && account.username && (
                <Link
                  href="/account/change-plan"
                  className="block w-full px-2 py-1.5 text-[10px] font-medium text-center text-white bg-gray-900 hover:bg-gray-800 rounded-md transition-colors"
                >
                  Upgrade
                </Link>
              )}
