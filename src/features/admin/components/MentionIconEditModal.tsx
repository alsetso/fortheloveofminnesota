...
interface MentionIconEditModalProps {
  isOpen: boolean;
...
export default function MentionIconEditModal({
  isOpen,
  icon,
  onClose,
  onSave,
}: MentionIconEditModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const response = await fetch(`/api/admin/mention-icons/${icon.id}`, {
        method: 'PUT',
      ...
      alert(error instanceof Error ? error.message : 'Failed to update mention icon');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="fixed inset-0 z-[101] flex items-center justify-center pointer-events-none p-4">
        ...
        >
          ...
          <div className="sticky top-0 bg-white border-b border-gray-200 px-[10px] py-3 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-gray-900">Edit Mention Icon</h2>
            <button
              onClick={onClose}
              disabled={saving}
