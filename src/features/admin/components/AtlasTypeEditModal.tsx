...
interface AtlasTypeEditModalProps {
  isOpen: boolean;
  type: AtlasType | null;
  onClose: () => void;
  onSave: () => void;
...
export default function AtlasTypeEditModal({
  isOpen,
  type,
  onClose,
  onSave,
}: AtlasTypeEditModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const response = await fetch(`/api/admin/atlas-types/${type.id}`, {
        method: 'PUT',
      ...
      alert(error instanceof Error ? error.message : 'Failed to update atlas type');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      setFormData((prev) => ({ ...prev, icon_path: data.path }));
    } catch (error) {
      ...
      <div className="fixed inset-0 z-[101] flex items-center justify-center pointer-events-none p-4">
        ...
        >
          ...
          <div className="sticky top-0 bg-white border-b border-gray-200 px-[10px] py-3 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-gray-900">Edit Atlas Type</h2>
            <button
              onClick={onClose}
              disabled={saving}
          ...
          <form onSubmit={handleSubmit} className="p-[10px] space-y-3">
            <div>
              ...
              <p className="mt-1 text-[10px] text-orange-600">
                Warning: Changing the slug will break existing links and map layers if it doesn't match the actual table name.
              </p>
            </div>
