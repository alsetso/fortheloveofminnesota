-- Fix extract_articles_from_prompt to handle both camelCase and snake_case field names
-- RapidAPI might return different field names than expected
...
BEGIN
  ...
  LOOP
    BEGIN
      -- Get article title for error messages (try multiple field name variations)
      v_article_title := COALESCE(
        v_article->>'title',
        v_article->>'Title',
        'Untitled'
      );
      
      -- Parse publishedAt (try multiple field name variations)
      BEGIN
        v_published_at := COALESCE(
          (v_article->>'publishedAt')::TIMESTAMP WITH TIME ZONE,
          (v_article->>'published_at')::TIMESTAMP WITH TIME ZONE,
          (v_article->>'publishedAt')::TIMESTAMP WITH TIME ZONE,
          v_prompt.created_at
        );
      EXCEPTION
      ...
      -- Insert article (handle both camelCase and snake_case field names)
      INSERT INTO news.generated (
        ...
        COALESCE(
          ...
          v_article->'source'->>'source_url'
        ),
        COALESCE(
          v_article->'source'->>'name',
          v_article->'source'->>'source_name'
        ),
        COALESCE(
          v_article->'source'->>'logoUrl',
          v_article->'source'->>'logo_url'
        ),
        COALESCE(
          v_article->'source'->>'faviconUrl',
          v_article->'source'->>'favicon_url'
        ),
        v_article->'source'->>'publicationId',
        COALESCE(
          v_article->'relatedTopics',
          v_article->'related_topics',
          '[]'::jsonb
        )
      )
  ...
  IF v_failed_count > 0 THEN
    RAISE WARNING 'Extracted % articles, failed to insert % articles for prompt %', 
      v_count, v_failed_count, p_prompt_id;
  END IF;
...
COMMENT ON FUNCTION news.extract_articles_from_prompt IS 
  'Extracts articles from prompt and inserts into generated table. Handles both camelCase and snake_case field names. Returns count.';
