...
/** Plan slugs that can access Work (contributor or higher). */
export const WORK_REQUIRED_PLANS = ['contributor', 'plus', 'gov'] as const;

const ACTIVE_SUBSCRIPTION_STATUSES = ['active', 'trialing'];

export function useWorkAccess(): { hasAccess: boolean; isLoading: boolean } {
  const { account, isAccountLoading } = useAuthStateSafe();

  const hasAccess =
    !!account &&
    WORK_REQUIRED_PLANS.includes(account.plan as (typeof WORK_REQUIRED_PLANS)[number]) &&
    ACTIVE_SUBSCRIPTION_STATUSES.includes(account.subscription_status || '');

  return { hasAccess, isLoading: isAccountLoading };
}

export default function WorkMapGate({ children }: { children: React.ReactNode }) {
  const { user, isLoading: authLoading } = useAuthStateSafe();
  const { hasAccess, isLoading: accountLoading } = useWorkAccess();
  ...
  const planOverlay = showPlanOverlay && (
    ...
    >
      ...
      >
        <p className="text-sm font-semibold text-gray-900 dark:text-foreground">
          Work requires a Contributor or higher plan
        </p>
        <p className="text-xs text-gray-600 dark:text-foreground-muted max-w-[240px]">
          Upgrade to access the map and workforce tools.
        </p>
        <div className="flex items-center gap-2 flex-wrap justify-center">
          <Link
            href="/settings/plans"
