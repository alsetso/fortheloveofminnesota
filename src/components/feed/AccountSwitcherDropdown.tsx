...
export default function AccountSwitcherDropdown({
  ...
  useEffect(() => {
    const loadAccount = async () => {
      ...
      try {
        if (user) {
          // Authenticated user - get their account
        ...
        console.error('[AccountSwitcherDropdown] Error loading account:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  // Only for authenticated accounts - callback or default behavior
  ...
  // Display account (only authenticated)
