...
/** Account dock pane — identity + core game hubs, inside the dock body shell. */
export default function DockAccountPane() {
  const router = useRouter();
  const { openDockCard, openProfileCard, collapse } = useMapDock();
  const {
    account,
    accounts,
    user,
  ...
  const onResetup = async () => {
    ...
    try {
      ...
      // Gate catches any lingering complete state on refresh.
    }
  ...
  const onSignOut = async () => {
    // Collapse dock immediately so the user cannot interact further.
    collapse();
    await signOut();
