...
export default function PostMapModal({
  ...
  const addPinToCanvas = useCallback(async (mapInstance: import('mapbox-gl').Map, lng: number, lat: number, hidePin: boolean) => {
    const sourceId = 'pin-source';
    const layerId = 'pin-layer';
    const imageId = 'heart-pin-image';
    ...
    if (!hidePin) {
      ...
      if (!mapInstance.hasImage(imageId)) {
        try {
          ...
          console.error('[PostMapModal] Failed to load heart icon:', error);
        }
      }

      // Add symbol layer with heart image
  ...
  const handleSuggestionSelect = useCallback(async (suggestion: MapboxSuggestion) => {
    ...
    if (!state.isDrawing && draw.current && (draw.current.getMode?.() || '') !== 'draw_polygon' && map.current) {
      ...
      // Add pin to canvas
      await addPinToCanvas(map.current, lng, lat, state.hidePin);
        ...
        mapInstance.on('load', async () => {
          ...
          if (initialMapData) {
            ...
            if ((initialMapData.type === 'pin' || initialMapData.type === 'both') && initialMapData.geometry.type === 'Point') {
              ...
              // Add pin to canvas
              await addPinToCanvas(mapInstance, lng, lat, initialMapData.hidePin || false);
          ...
          let pendingPolygonCompletion: GeoJSON.Polygon | null = null;
            ...
            if (!state.isDrawing && drawInstance.getMode() !== 'draw_polygon') {
              ...
              // Add pin to canvas
              await addPinToCanvas(mapInstance, lng, lat, state.hidePin);
