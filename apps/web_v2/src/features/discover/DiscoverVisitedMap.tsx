...
/**
 * Full-bleed inline map of visited territory boundaries for a Discover kind page.
 *
 * Frame crops the bottom 2rem of the Mapbox canvas so the visible composition
 * reads tighter; fitBounds / overlays use the cropped frame, not the full canvas.
 */
...
const SOURCE_ID = 'discover-visited';
const FILL_ID = 'discover-visited-fill';
const LINE_ID = 'discover-visited-line';
...
type VisitedBoundariesResponse = FeatureCollection & {
  meta?: { visitedTotal?: number; label?: string; unitKind?: string };
};

export type DiscoverVisitedMapSelect = {
  id: string;
  name: string;
};
...
export function DiscoverVisitedMap({
  kindSlug,
  label,
  onSelect,
}: {
  kindSlug: string;
  label: string;
  onSelect?: (row: DiscoverVisitedMapSelect) => void;
}) {
    ...
    void (async () => {
      try {
        const res = await fetch(
          `/api/discover/${encodeURIComponent(kindSlug)}/visited-boundaries`,
          { credentials: 'include', cache: 'no-store', signal: ac.signal },
        );
        if (!res.ok) throw new Error('boundaries');
        const body = (await res.json()) as VisitedBoundariesResponse;
      ...
      {empty || error ? (
        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-[2] px-4 pb-4 pt-12 text-center [background:linear-gradient(to_top,rgba(40,32,24,0.42)_0%,transparent_100%)]">
          <p className="text-[14px] font-semibold text-white">
            {error ? 'Couldn’t load map layers' : 'Nothing on the map yet'}
          </p>
          <p className="mt-0.5 text-[12px] text-white/85">
            {error
              ? 'Your lists below still work.'
              : 'Find Me in Minnesota to start exploring.'}
          </p>
        </div>
      ) : null}
    </div>
