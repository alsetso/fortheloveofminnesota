...
import BuildingPageClient from '@/app/gov/building/[slug]/BuildingPageClient';
import CopyButton from '@/app/gov/building/[slug]/CopyButton';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const hasBranch = data.agencies.some((a) => a.branch === branch);
  if (!hasBranch) {
    return { title: 'Not Found', robots: { index: false, follow: false } };
  }
  const { building } = data;
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://fortheloveofminnesota.com';
  const url = `${baseUrl}/gov/${branch}/building/${slug}`;
  const title = `${building.name ?? 'Building'} | Minnesota Government`;
  const description =
    building.description ??
    `${building.name ?? 'Civic building'} — ${building.full_address ?? 'Minnesota'}.`;
  return {
    title,
    description,
    openGraph: {
...
export default async function BranchBuildingPage({ params }: Props) {
  const { branch, slug } = await params;
  ...
  const hasBranch = data.agencies.some((a) => a.branch === branch);
  if (!hasBranch) notFound();

  const { building, people, agencies } = data;
    ...
    <div className="max-w-4xl mx-auto px-[10px] py-3">
      ...
      <div className="mb-3 space-y-2">
        <div className="flex items-start justify-between gap-2 flex-wrap">
          ...
          <BuildingPageClient building={building} isAdmin={isAdmin} branch={branch} />
        </div>

        {isAdmin && (
          <LastEditedIndicator tableName="buildings" recordId={building.id} />
        )}
