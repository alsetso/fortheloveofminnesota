...
export default function GeneralSettingsClient() {
  ...
  const handleManageBilling = () => router.push('/billing');
    ...
    <>
        ...
        <div className="flex items-center justify-between p-[10px] border border-gray-200 rounded-md hover:bg-gray-50 transition-colors">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-600">
              {isProUser ? (isActive ? `${planPrice} • Active subscription` : account.subscription_status === 'canceled' ? 'Subscription canceled' : account.subscription_status === 'past_due' ? 'Payment required' : 'Subscription inactive') : 'Upgrade to unlock Contributor features'}
            </p>
          </div>
          <button onClick={handleManageBilling} className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-gray-900 bg-white border border-gray-300 hover:bg-gray-50 rounded-md transition-colors flex-shrink-0">
      ...
      <div className="bg-white border border-gray-200 rounded-md p-[10px]">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button onClick={() => setShowBusinessModal(true)} className="p-[10px] border border-gray-200 rounded-md hover:bg-gray-50 transition-colors text-left">
            <div className="flex items-center justify-between mb-1.5">
              <h4 className="text-xs font-semibold text-gray-900">Business</h4>
              {account.plan === 'business' && <span className="px-1.5 py-0.5 text-[10px] font-medium text-gray-700 bg-gray-100 rounded-full">Current</span>}
            </div>
            <p className="text-[10px] text-gray-600 mb-2">Connect your business with Minnesota. Verified profiles and statewide visibility.</p>
            <span className="text-[10px] font-medium text-gray-900">Set up →</span>
          </button>
          <button onClick={() => setShowGovernmentModal(true)} className="p-[10px] border border-gray-200 rounded-md hover:bg-gray-50 transition-colors text-left">
            <div className="flex items-center justify-between mb-1.5">
              <h4 className="text-xs font-semibold text-gray-900">Government</h4>
              {account.plan === 'gov' && <span className="px-1.5 py-0.5 text-[10px] font-medium text-gray-700 bg-gray-100 rounded-full">Current</span>}
            </div>
            <p className="text-[10px] text-gray-600 mb-2">Help your residents love Minnesota more. Strategic initiatives and civic engagement tools.</p>
            <span className="text-[10px] font-medium text-gray-900">Set up →</span>
          </button>
        </div>
      </div>
      ...
      <DraggableBottomSheet isOpen={showBusinessModal} onClose={() => setShowBusinessModal(false)} initialHeight={90} snapPoints={[50, 90]} showCloseButton={false} showDragHandle={false} hideScrollbar={true} contentClassName="p-0" sheetClassName="w-full max-w-[700px] rounded-t-2xl" centered={true}>
        <BusinessSetupForm onBack={() => setShowBusinessModal(false)} />
      </DraggableBottomSheet>
      <DraggableBottomSheet isOpen={showGovernmentModal} onClose={() => setShowGovernmentModal(false)} initialHeight={90} snapPoints={[50, 90]} showCloseButton={false} showDragHandle={false} hideScrollbar={true} contentClassName="p-0" sheetClassName="w-full max-w-[700px] rounded-t-2xl" centered={true}>
